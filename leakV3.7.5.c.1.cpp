...
#define CY_RANGE 5                      //is no of y or y/2 pixel uncertain between any two frame of the game
#define CX_RANGE 2                      //is no of x or x/2 pixel uncertain between any two frame of the game
#define RNX_BUBB 7                      //how many no of Rxn time expirement to be performed
#define START_SAVING 5
#define GRAPH_INIT initwindow(SCREEN_WIDTH,SCREEN_HEIGHT,TITLE)
...
typedef struct addtional_info           //structure to store the additional information about state of game
{
    ...
    bool prev_click;                     //for score pani na badhaune out ni nagaraune
...
bool D_h_s_sort()                    //returns 0 if sucessfull
{
    int struct_size, i, j;
    h_s_d h1, h2;
...
bool save_h_s_iff(int score)       //save high score if and only if
{
    h_s_d h;
...
int saving_keeper(int change,s **sp,inf *inf1)
{
    static int saving = START_SAVING;    //saving in numbers
    int i;
    char savin[16];
    saving += change;
    for(i=inf1->saver2;i<inf1->indi;i++)
    {
        if((((sp[i]->curr_x +sp[i]->c) - (SCREEN_WIDTH+oTARGET_WIDTH)/2)) > (sp[i]->asize))
        {
            (inf1->saver2)++;
            if((sp[i]->color)!= 15)       saving--;
        }
    }
    sprintf(savin,"Saving: %d",saving);
    settextstyle(4,0,3);
    b_but(0,textheight(savin),savin,BLACK);
    return saving;
}
int score_keeper(int change)
{
...
int UP_SCORE(s **sp,inf *inf1,int t_b)        //this function return the increment score accoring to distance of mouse pointer from center
{
    int d;                          //distance
    int score;
...
void y_uncer_val(s **sp,int i)
{
    ...
    if((sp[i]->curr_y + dy) > ((SCREEN_HEIGHT+TARGET_HEIGHT)/2 - sp[i]->asize))       // edi taget box bhanda mathi gayo bhane tellai count nagarna ko lagi tyo value tala arkai assign  // over do cases
        dy = -rnd(0,CY_RANGE);
...
void display_over(char *reason,bool page)
{
    setvisualpage(page);
    cleardevice();
...
{
    int i;
    bool b_t_r;     //value is 1 when within target box   // is boolen variable for knowing if the mouse is witnin the target box
    static bool page=0;
    setactivepage(page);//drawing page
    setvisualpage(1-page);  //display page
    cleardevice();
    setlinestyle(0,0,2);
    if(!(GetAsyncKeyState(VK_LBUTTON) || GetAsyncKeyState(VK_RBUTTON)))
        inf1->prev_click = 0;                                             //reset prev_click if continousclick has stoped
    for(i=(inf1->saver1);i<(inf1->indi);i++)
    {
        setcolor(WHITE);
        setfillstyle(sp[i]->fill,sp[i]->color);
    ...
    {
        display_over("Out of Saving",page);
        return 0;
    }
    ...
    {
            display_over("No Balloon there",page);
            return 0;
    }
    if((GetAsyncKeyState(VK_LBUTTON) || GetAsyncKeyState(VK_RBUTTON))&&((inf1->ssh)++) == 0)
    {
        if(GetAsyncKeyState(VK_LBUTTON)&& (((sp[t_b]->asize == sr) && (sp[t_b]->fill == 1)) || ((sp[t_b]->asize == br) && (sp[t_b]->fill == 5))) && b_t_r && inf1->prev_click==0)
        {
            setcolor(sp[t_b]->color = 15);      //disappear the bubble(white)
            score_keeper(UP_SCORE(sp,inf1,t_b));     //score up
            inf1->prev_click = 1;               //after clicking correct, turn on the booloen to freeze game state until he/sheleaves the click button
        }

        else if(GetAsyncKeyState(VK_RBUTTON) && (((sp[t_b]->asize == br) && (sp[t_b]->fill == 1)) || ((sp[t_b]->asize == sr) && (sp[t_b]->fill == 5))) && b_t_r && inf1->prev_click==0)
        {
            setcolor(sp[t_b]->color = 15);      //disappear the bubble
            score_keeper(UP_SCORE(sp,inf1,t_b));
            inf1->prev_click = 1;               //after clicking correct, turn on the booloen to freeze game state until he/sheleaves the click button
        }
        else if(!b_t_r)
        {
            display_over("Your cursor is outside target box",page);
            return 0;                           //this function returns 0 if game is over
        }

        else if(inf1->prev_click==0)
            {
                display_over("Wrong Click for ballon",page);
                return 0;
            }
    }
    page = 1-page;
    delay(FRAME_DEL);
    return 1;                                   //it returns 1 if nothing happened
}
void target_box()
{
    int score = score_keeper(0);
    if(oTARGET_WIDTH > SCREEN_WIDTH/10)  oTARGET_WIDTH = SCREEN_WIDTH/3 - score;
    setcolor(RED);
    setlinestyle(0,0,3);
...
void new_game()
{
    ...
    inf inf1;
    inf1.ssh = inf1.saver1 = inf1.prev_click= inf1.saver2 = 0;
    int i,GAP;
    setbkcolor(WHITE);
    for(i=0;i<NO_BU;i++)
    {
        ...
        sp[i]->fill = (rnd(1,100)%17 == 0) ? 5 : 1;     //5 is stripe pattern of some balloon in setfillstyle
        for(sp[i+1]->c=0,sp[i+1]->asize=rd(),sp[i]->color=rnd(0,14),inf1.indi=i+1,sp[i]->cy = 0,sp[i]->cx = 0,sp[i]->curr_y=ycen,sp[i+1]->curr_x=xcen;;)     //i is the identifier of each bubblne here
        {
            ...
            if((sp[i]->c)>(GAP))      //c for counter  //do not generqate until minimum gap size is there ...baloon is created when c for each balloon increases till gap size
                break;                //after break, index i increase
            if(bu_gen(sp, &inf1) != 1)
            {   free_rem_sp(sp,inf1.saver1,i+1);        //free all the memory remainig to free up
                ...
                saving_keeper(START_SAVING-saving_keeper(0,sp,&inf1),sp,&inf1);   //reset saving
...
bool button(int y, char *but_nam)
{
    ...
    txt_color = b_bool ? 9:0;   //green or black
    ...
    return 0;                                                           //0 for unsuccessfulclick
}
int welcom()
{
    GRAPH_INIT;
    ...
    for(char ne_gam[] = "New Game",high_sc[] = "High Score",close[] = "Exit",inst[] = "Instructions",mini_game[] = "Reflex Game";;)
    {
        //buttons
...
bool instruction()
{
    char inst[] = "Instructions", line[][61] = {"Place mouse pointer inside Targetbox", "Click Left mouse button for Small Bubble","Click Right mouse button for Big Bubble","Reverse is true if bubble is hatched","Click it as close to the center of Bubble", "Game is Over when you misclick it","Game is Over if Saving is zero","Saving decreases if bubble is ignored","Bonus Saving, if center of bubble is clicked","Rxn time is game to measure reflex time"};
    int i;
    GRAPH_INIT;
    setbkcolor(WHITE);
    floodfill(SCREEN_WIDTH-1,SCREEN_HEIGHT-1,WHITE);    // for white background
...
int main()
{
    ...
    for(;;)
    {
        ...
        switch(r)
        {
            ...
            case -1: closegraph();  return(0);    //when exit is clicked on welcome screen
        }
    }
    closegraph();
    return 0;
