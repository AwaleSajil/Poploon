    ...
    for(i=0;fread(&h2,sizeof(h2),1,fptr)==1;i++)
    {
        if(h2.score < score)
        {
            if(delete_data(h2.score) == 0)
                printf("Could not svae high score\n");
            fclose(fptr);
            return 1;
        }
    }
...
int welcom()
{
    ...
    int sep;
    int i,j, sum=0,new_txt_color=0,high_txt_color=0;
    int n_g_bo,h_s_bo;
    ...
    // names
        ...
        {
            setcolor(i+2);
            outtextxy((i+1)*sep+sum,SCREEN_HEIGHT-2*textheight(we[0]),we[i]);
            sum += textwidth(we[i]);
        }
        setcolor(5);
...
// for title
        settextstyle(3,0,7);
        outtextxy(SCREEN_WIDTH/2-textwidth(title)/2,SCREEN_HEIGHT/10,title);
    for(j=0;;j++)
    {
        //options
        settextstyle(8,0,5);
        setcolor(new_txt_color);
        outtextxy(SCREEN_WIDTH/2-textwidth(ne_gam)/2,SCREEN_HEIGHT/2.1-textheight(ne_gam),ne_gam);
        ...
        setcolor(high_txt_color);
        outtextxy(SCREEN_WIDTH/2-textwidth(high_sc)/2,SCREEN_HEIGHT/2,high_sc);
...
void print_h_s()
{
    ...
    settextstyle(8,0,5);
    setcolor(0);
    outtextxy(SCREEN_WIDTH/2-textwidth(h_s_s)/2,textheight(h_s_s)/2,h_s_s);
    settextstyle(8,0,3);
    for(i=0;fread(&h1,sizeof(h1),1,fptr) == 1; i++)     //fread returns 1 if they were sucessful
    {
        ...
        outtextxy(SCREEN_WIDTH/2-textwidth(buffer)/2,1.5*textheight(h_s_s)+(i+1)*textheight(buffer),buffer);
    }
    fclose(fptr);
}
int main()
