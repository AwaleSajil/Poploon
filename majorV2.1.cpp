...
int check_w_b(s **sp, inf *inf1)
{
    ...
    for(i=(inf1->saver1);i<(inf1->indi);i++)
    {
        ...
        if(oa<=t && oa>=(-t) && sp[i]->color != 15)     // sp[i]->color != 15 bubble isnot white color
        {
            if(prev_i != i)
    ...
    if(local_mouse_boln == (GetAsyncKeyState(VK_LBUTTON) || GetAsyncKeyState(VK_RBUTTON)))
    {
        local_mouse_boln = (GetAsyncKeyState(VK_LBUTTON) || GetAsyncKeyState(VK_RBUTTON));
        return 1;
    }
