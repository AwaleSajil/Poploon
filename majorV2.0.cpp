...
int save_h_s_iff(int score, int mode)
{
    h_s_d h;
...
void over(int score,char reason[])
{
    ...
    {
        if(save_h_s_iff(score, 0) == 1)      //mode 0 is stright append/ write mode
        {
            bkcolor = 4; //red
        }

    }
    else if(save_h_s_iff(score, 1) == 1)  // mode 1 is to first check if it is high socre if it is replace with the lowest high score
    {
