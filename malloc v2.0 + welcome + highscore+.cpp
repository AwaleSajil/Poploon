...
int delete_data(int h2score)
{
	...
	if(remove(H_S_LOCATION) != 0)   puts("Couldnot remove H_S_LOCATION");                //returning -1
	if(rename(TMP, H_S_LOCATION) !=0)    puts("Couldnot rename TMP");              // returning -1
	return 1;
}
int is_it_h_s(int score)
{
    ...
    fclose(fptr);
    D_h_s_sort(-1);      //reset the order to descending (optional)
    return 0;
}
void over(int score,char reason[])
{
    ...
    if(check_h_m_h_s_i_f() < 11)
    {
        if(save_h_s(score))
        {
            ...
            setcolor(15); // white
            settextstyle(3,0,2);
            outtextxy(SCREEN_WIDTH/2-textwidth(ha)/2,SCREEN_HEIGHT-1*textheight(ha),ha);
        }

    }
    else if(is_it_h_s(score))        // see if it is on top ten(say) high score
    {
        if(save_h_s(score))
            bkcolor = 4; //red
    }
...
int print_h_s()
{
    ...
    for(;;)
    {
        ...
        GetCursorPos(&m_po);
        boln = (m_po.x) > (SCREEN_WIDTH/2-textwidth(g_b_w_s)/2) && (m_po.x) < (SCREEN_WIDTH/2+textwidth(g_b_w_s)/2) && (m_po.y+15) > (SCREEN_HEIGHT-1.5*textheight(g_b_w_s)) && (m_po.y+15) > (SCREEN_HEIGHT-0.5*textheight(g_b_w_s));
