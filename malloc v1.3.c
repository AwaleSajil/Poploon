...
void bu_gen(s *s1,inf *inf1)        // this function is for animation/ creation of each(all) bullune on a frame
{
...
int frameloop(int i,s *s1,inf *inf1)
{
    int GAP;
    if(i==0)
            s1[i].asize=rd();
        ...
        {
            int j;
            if(s1[i].c==1)      //if it is 1st time for that bullone initilize GAP
            {
            ...
            for(j=(inf1->saver1);j<=i;j++)           //all the necessery increments for making all booluns moving
                s1[j].c++;
            if((xcen+s1[inf1->saver1].c)> (getmaxx()+br))
                (inf1->saver1)++;
        }
...
int main()
{
    ...
    s s1[BU_LIM];      // decalre array of type struct screen
    inf inf1;
    ...
    for(i=0;i<BU_LIM;i++)
    {
        frameloop(i,s1,&inf1);
            ...
            (inf1.saver1) -= 2;                 //if(i == BU_LIM-1)   assuming inf1.saver1  atleast 2
            frameloop(0,s1,&inf1);
   getch();
   closegraph();
   return 0;
