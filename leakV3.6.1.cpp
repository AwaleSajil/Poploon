...
typedef struct addtional_info
{
    ...
    int saver1;     // for CPU efficency how many balloons have gone out
    int ssh;        // ssh --> stop scoring high
    unsigned int prev_click:1;   //for score pani badhaune out ni nagaraune
}inf;

// prototype area
void over(int score,char *reason);
int score_keeper(short int change);
...
void b_but(short int x, short int y, char str[], short int color)
{
    setcolor(color);
    outtextxy(x,y,str);
}
...
{
    short int dy = rnd(-2*CY_RANGE,2*CY_RANGE); //randomize y cordinates;
...
{
    short int i;
    short int b_t_r;        // is boolen variable for knowing if the mouse is witnin the target box
    static short int page=0;
    setactivepage(page);
...
void new_game()
{
    ...
    for(i=0;i<NO_BU;i++)
    {
        ...
        for(sp[i+1]->c=1,sp[i+1]->asize=rd(),sp[i]->color=rnd(0,14),inf1.indi=i+1,sp[i]->cy = 0,sp[i]->cx = 0,sp[i]->curr_y=ycen,sp[i+1]->curr_x=xcen;;)     //i is the identifier of each bubblne here
        {
            ...
            if((sp[i]->c)>(GAP))      //c for counter  //do not generqate until minimumygapsize
                break;
            if(bu_gen(sp, &inf1) != 1)
            {
            ...
            {
                (sp[j]->c)+= step_process();//sp[j]->c controlsspeed i,e accelearating
            }
...
int ycalc(short int nt,short int n,short int font=8, short int txt_size=5)
{
    short int total_heit,y,ygap=10,txt_heit;
...
int welcom()
{
    ...
    int nt = 4;     //no of buttons
        ...
        // names
        {
            ...
            for(i=0;i<4;i++)
            {
                b_but(sum+sep,SCREEN_HEIGHT-2*textheight(we[0]),we[i],i+2);
                sum +=(sep+textwidth(we[i]));
            }
        }
