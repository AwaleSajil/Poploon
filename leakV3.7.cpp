...
typedef struct addtional_info
{
    ...
    int prev_click;   //for score pani badhaune out ni nagaraune
}inf;
...
void new_game()
{
    ...
    for(i=0;i<NO_BU;i++)
    {
        ...
        for(sp[i+1]->c=0,sp[i+1]->asize=rd(),sp[i]->color=rnd(0,14),inf1.indi=i+1,sp[i]->cy = 0,sp[i]->cx = 0,sp[i]->curr_y=ycen,sp[i+1]->curr_x=xcen;;)     //i is the identifier of each bubblne here
        {
            ...
            for(j=(inf1.saver1);j<(inf1.indi);j++)           //all the necessery increments for making all booluns moving
            {
...
int welcom()
{
    ...
    int nt = 4;     //no of buttons
        ...
        // names
        {
            ...
            for(i=0;i<4;i++)
            {
                b_but(sum+sep,SCREEN_HEIGHT-1.5*textheight(we[0]),we[i],i+2);
...
int instruction()
{
    char inst[] = "Instructions", line[][61] = {"Place mouse pointer inside Targetbox", "Click Left Click for Small Bubble","Click Right Click for Big Bubble", "Click it as close to the center of bubble"};
    int i;
    initwindow(SCREEN_WIDTH,SCREEN_HEIGHT,TITLE,true,true);
    setbkcolor(WHITE);
    floodfill(SCREEN_WIDTH-1,SCREEN_HEIGHT-1,WHITE);    // for white background
