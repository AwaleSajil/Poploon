...
void over(int score,char reason[])
{
    ...
    char sc[15];
    int bkcolor = 0; //black
    if(!(D_h_s_sort()) == (save_h_s_iff(score)) )      //note () have associtivity of left to right
        bkcolor = 4; //red
    setbkcolor(bkcolor);
    settextstyle(3,0,8);
...
int bu_gen(s **sp,inf *inf1)
{
    ...
    b_t_r = ((m_po.x)>=((SCREEN_WIDTH-oTARGET_WIDTH)/2) && (m_po.x)<=((SCREEN_WIDTH+oTARGET_WIDTH)/2) && (m_po.y-15)>=(ycen-TARGET_HEIGHT/2) && (m_po.y)<=(ycen+TARGET_HEIGHT/2));
...
void target_box()
{
    setlinestyle(1,0,1);
    setcolor(RED);
    line(SCREEN_WIDTH/2,ycen-TARGET_HEIGHT/2,SCREEN_WIDTH/2,ycen-TARGET_HEIGHT/2+TARGET_HEIGHT/7);      //vertical line obserable
    line(SCREEN_WIDTH/2,ycen+TARGET_HEIGHT/2,SCREEN_WIDTH/2,ycen+TARGET_HEIGHT/2-TARGET_HEIGHT/7);      //vertical line obserable
    line(SCREEN_WIDTH/2-oTARGET_WIDTH/2,ycen,SCREEN_WIDTH/2-oTARGET_WIDTH/2+oTARGET_WIDTH/3,ycen);      //horizontal line
    line(SCREEN_WIDTH/2+oTARGET_WIDTH/2,ycen,SCREEN_WIDTH/2+oTARGET_WIDTH/2-oTARGET_WIDTH/3,ycen);      //horizontal line
    setlinestyle(0,0,3);
