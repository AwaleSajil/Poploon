...
int UP_SCORE(s **sp, int t_b)
{
    ...
    int score;
    GetCursorPos(&m_po);
    d = sqrt(pow((sp[t_b]->curr_x +sp[t_b]->c) - (m_po.x),2) + pow((sp[t_b]->curr_y) - (m_po.y-15),2));
...
int check_w_b(s **sp, inf *inf1)
{
    ...
    for(i=(inf1->saver1);i<(inf1->indi);i++)
    {
        ...
        int d = sqrt(pow((sp[i]->curr_x +sp[i]->c) - (m_po.x),2) + pow((sp[i]->curr_y) - (m_po.y-15),2));    //distance between pointer and bubble center
            if(d<=t && d>=(-t))
            {
                if(prev_i != i)
...
void x_uncer_val(s **sp,int i)
{
    ...
    if(((sp[i-1]->curr_x+sp[i-1]->c)-(sp[i]->curr_x +sp[i]->c+ dx))   <   (sp[i]->asize + sp[i-1]->asize))
        dx = rnd(-CX_RANGE,0);
    if(((sp[i]->curr_x +sp[i]->c+ dx)-(sp[i+1]->curr_x+sp[i+1]->c))   <   (sp[i]->asize + sp[i+1]->asize))         // overdo cases
        dx = rnd(0,CX_RANGE);
    sp[i]->cx = dx;
}
int bu_gen(s **sp,inf *inf1)
{
    ...
    for(i=(inf1->saver1);i<(inf1->indi);i++)
    {
        ...
        line(sp[i]->curr_x,sp[i]->curr_y+sp[i]->asize,sp[i]->curr_x,sp[i]->curr_y+sp[i]->asize+sr*1.5); //string of bubble
...
void new_game()
{
    ...
    for(i=0;i<NO_BU;i++)
    {
        ...
        for(sp[i]->c=1,sp[i+1]->asize=rd(),sp[i]->color=rnd(0,14),inf1.indi=i+1,sp[i]->cy = 0,sp[i]->cx = 0,sp[i]->curr_y=ycen,sp[i+1]->curr_x=xcen;;)     //i is the identifier of each bubblne here
        {
            int j;
