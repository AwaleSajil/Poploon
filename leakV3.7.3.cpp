...
void target_box()
{
    ...
    if(oTARGET_WIDTH <= SCREEN_WIDTH/10)  oTARGET_WIDTH = rnd(20,70);
    setcolor(RED);
    setlinestyle(0,0,3);
...
bool instruction()
{
    char inst[] = "Instructions", line[][61] = {"Place mouse pointer inside Targetbox", "Click Left mouse button for Small Bubble","Click Right mouse button for Big Bubble","Click it as close to the center of Bubble", "Game is Over when you misclick it", "Rxn time is game to measure reflex time"};
    int i;
    initwindow(SCREEN_WIDTH,SCREEN_HEIGHT,TITLE);
    setbkcolor(WHITE);
    floodfill(SCREEN_WIDTH-1,SCREEN_HEIGHT-1,WHITE);    // for white background
